...
export const getFunctionDefinitionTool = {
    ...
    execute: async (input: GetFunctionDefinitionInput, context: ToolContext) => {
        ...
        let whereClause = `n.nspname = '${schema.replace(/'/g, "''")}' AND p.proname = '${function_name.replace(/'/g, "''")}'`;

        if (argument_types) {
            // Use pg_get_function_arguments to match the argument signature
            whereClause += ` AND pg_catalog.pg_get_function_arguments(p.oid) = '${argument_types.replace(/'/g, "''")}'`;
        }
